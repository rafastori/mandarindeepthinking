...
export const ActiveGame: React.FC<ActiveGameProps> = ({ room, user, onResult }) => {
    ...
    return (
        <div className="text-center animate-in fade-in slide-in-from-bottom-4 relative pb-10">
            ...
            {/* CENA 1: Usuário sem carta (Precisa pegar uma OU aguardando cartas) */}
            {!myCard && (
                <div className="py-12 flex flex-col items-center">
                    {room.status === 'regenerating' ? (
                        // Gerando novas cartas
                        <>
                            <div className="w-24 h-32 bg-gradient-to-br from-purple-100 to-purple-50 border-2 border-purple-200 rounded-xl flex items-center justify-center mb-6 shadow-sm animate-pulse">
                                <Icon name="sparkles" className="text-purple-400 animate-spin" size={32} />
                            </div>
                            <h3 className="text-xl font-bold text-purple-700 mb-2">Criando novas cartas...</h3>
                            <p className="text-slate-500 mb-4 max-w-xs">A IA está gerando mais palavras para o jogo.</p>
                    ...
                    ) : (room.cardQueue?.length || 0) === 0 ? (
                        // Sem cartas na fila (aguardando geração)
                        <>
                            <div className="w-24 h-32 bg-slate-100 border-2 border-dashed border-slate-300 rounded-xl flex items-center justify-center mb-6 shadow-sm">
                                <Icon name="loader" className="text-slate-400 animate-spin" size={32} />
                            </div>
                            <h3 className="text-xl font-bold text-slate-600 mb-2">Aguardando cartas...</h3>
                            <p className="text-slate-500 max-w-xs">As cartas estão sendo preparadas.</p>
                        </>
                    ) : (
                        // Normal: pegar carta
                        <>
